...
namespace MatchingEngine
{
    public static class Run
    {
            ...
            bool stop_at_first_match = true, bool exact_matches_allowed = false, double lower_score_treshold = 0.70)
        {
            ...
            while (i < index1)
            {
                ...
                while (j < index2 & no_match_found_yet)
                {
                    ...
                    if (Helpers.FirstCharactersAreEqual(Records1[i].PatientInfo.FirstName,
                        Records2[j].PatientInfo.FirstName))
                    {
                        ...
                        temp_dist = DistanceVector.CalculateDistance(Records1[i].PatientInfo, Records2[j].PatientInfo);
            ...
            bool stop_at_first_match = true, bool exact_matches_allowed = false, double lower_score_treshold = 0.70)
        {
            ...
            log.WriteLine("Now we wil look for any matches from " + filepath1 + " with " + filepath2 +
                          " filtering by first character of FirstName and" + optional_not +
            ...
            elapsedTime = String.Format("{0:00}:{1:00}:{2:00}.{3:000}", ts.Hours, ts.Minutes, ts.Seconds,
                ts.Milliseconds);
            log.WriteLine("RunTime for matching " + filepath1 + " (from " + start_index1.ToString() + " to index " +
                          end_index1.ToString() + ") to " + filepath2 + " (from " + start_index2.ToString() +
                          " to index " + end_index2.ToString() + ") took:  " + elapsedTime);
